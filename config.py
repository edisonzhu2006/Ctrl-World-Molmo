...
class wm_args:
    ...
    dataset_stat_cfgs = None
    # dataset mix sampling:
    # - manual: use dataset_sampling_probs
    # - prorated_samples: proportional to number of samples in each dataset
    # - prorated_trajectories: proportional to unique episode_id count in each dataset
    dataset_sampling_mode = "prorated_trajectories"
    dataset_sampling_probs = None
    # When True, train DataLoader uses MixedDomainBatchSampler so every minibatch
    # contains samples from both datasets (see dataset_mix_counts).
    dataset_mix_batch: bool = False
    dataset_mix_counts: str = "1,3"  # counts in dataset_names order; must sum to train_batch_size
    dataset_mix_seed: int = 0
    prob=[1.0]
    annotation_name='annotation' #'annotation_all_skip1'
    num_workers=4
    down_sample=1 # if annotation in 15Hz: set to 3 downsample
    skip_step = 1
    

    # logs parameters
    debug = False
    tag = 'cotrain-droid-molmobot'
    output_dir = f"/scratch/gpfs/SHAHD/lh2004/model_ckpt/{tag}"
    wandb_run_name = tag
    wandb_project_name = "molmobot_example"


    # training parameters
    learning_rate= 5e-5  # hot peak for cluster bs=128 effective (4x colleague's bs=32 anchor, > linear-scale 4e-5)
    # LR schedule: linear warmup over lr_warmup_steps, then cosine decay to
    # lr_min_ratio * peak. Defaults sized for max_train_steps=100000.
    lr_warmup_steps = 3000  # 3% of 100k
    lr_min_ratio = 0.1
    ...
    def __post_init__(self):
        ...
        if self.task_type == "replay":
            ...
            self.val_id = [] # all episodes under annotation/val; else list explicit ids
