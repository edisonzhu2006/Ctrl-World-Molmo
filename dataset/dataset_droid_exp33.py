...
class Dataset_mix(Dataset):
    def __init__(
        ...
        kept_prob = []
        for i, (dataset_name, dataset_cfg) in enumerate(zip(dataset_names, dataset_cfgs)):
            ...
            # Mixture weights ∝ number of trajectories (unique episode_id), not rows in train_sample.
            traj_n = len({str(s["episode_id"]) for s in samples})
            kept_prob.append(float(traj_n))
            print(f"  trajectories (unique episode_id) cfg={dataset_cfg}: {traj_n}")

            # prepare normalization
            stat_path = f'{dataset_meta_info_path}/{dataset_name}/stat.json'
            if not os.path.exists(stat_path):
                stat_path = f'{dataset_meta_info_path}/stat.json'
            print(f"[{mode}] loading stat.json for cfg={dataset_cfg} name={dataset_name}: {stat_path}")
        ...
        prob_arr = np.array(kept_prob, dtype=np.float64)
        if (prob_arr < 0).any() or prob_arr.sum() <= 0:
            raise ValueError(f"Invalid prob after filtering for mode={mode}: {kept_prob}")
        self.prob = (prob_arr / prob_arr.sum()).tolist()
        self.max_id = max(self.samples_len)
        print('samples_len:', self.samples_len, 'max_id:', self.max_id, 'mix_prob (by traj count):', self.prob)
    ...
    def __getitem__(self, index):
        ...
        frame_len = np.floor(joint_len / 3) # downsample 15hz to 5hz
