...
class Action_encoder2(nn.Module):
    ...
    def forward(self, action,  texts=None, text_tokinizer=None, text_encoder=None, frame_level_cond=True,):
        # Disabled from torch.compile: this forward calls a HF tokenizer on
        # Python strings, which makes Dynamo specialize on string identity
        # and recompile on every new caption (hits recompile_limit, falls
        # back to eager). The action encoder is small and the text encoder
        # is frozen, so running this region eagerly costs ~nothing vs the
        # 1.5B-param UNet that stays compiled.
...
class CrtlWorld(nn.Module):
    def __init__(self, args):
        ...
        # Validate the UNet's expected added-time-embedding dim matches our
        # 3-element [fps, motion_bucket_id, noise_aug_strength] conditioning
        # vector, once at init rather than every forward. Mirrors the check
        # in StableVideoDiffusionPipeline._get_add_time_ids.
        expected_add_embed_dim = self.unet.add_embedding.linear_1.in_features
        passed_add_embed_dim = self.unet.config.addition_time_embed_dim * 3
        if expected_add_embed_dim != passed_add_embed_dim:
            raise ValueError(
                f"UNet expects added_time_embed dim {expected_add_embed_dim}, "
                f"got {passed_add_embed_dim} (3 × {self.unet.config.addition_time_embed_dim})."
            )

    def _get_add_time_ids(self, fps, motion_bucket_id, noise_aug_strength, dtype, batch_size):
        # Inlined from StableVideoDiffusionPipeline so training forward does
        # not need to reach into self.pipeline (which triggers Dynamo to walk
        # the inference pipeline's submodules and conflict with the separately
        # compiled pipeline.unet at val time).
    ...
    def forward(self, batch):
        ...
        fps = self.args.fps
        added_time_ids = self._get_add_time_ids(fps, motion_bucket_id, noise_aug_strength, action_hidden.dtype, bsz)
